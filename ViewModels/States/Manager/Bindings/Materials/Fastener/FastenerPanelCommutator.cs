...
using FenceFactory.Desktop.ViewModels.States.Manager.Visual.Materials.Fastener;

namespace FenceFactory.Desktop.ViewModels.States.Manager.Bindings.Materials.Fastener;

public class FastenerPanelCommutator
{
    private FastenerMaterialPanel? _view;
    ...
    public void Connect()
    {
        ...
        _view = mainPanel?.PanelContainer.Content as FastenerMaterialPanel;

        if (_view == null) return;

        // Локальная инициализация препарера и прямая настройка визуала
        var preparer = new FastenerPanelViewPreparer();
